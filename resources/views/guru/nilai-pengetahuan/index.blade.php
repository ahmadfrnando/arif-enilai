...
<div class="p-6 mt-14 space-y-4">
    <h1 class="text-3xl font-bold text-gray-900">Nilai Pengetahuan</h1>
    <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
        <div class="flex gap-4">
            <form method="GET" action="{{ route('guru.nilai-pengetahuan') }}" class="mx-auto flex gap-4">
            ...
            <a href="{{ route('guru.nilai-pengetahuan') }}" class="flex">
        ...
        <div class="relative">
            <form action="{{ route('guru.nilai-pengetahuan') }}" method="GET">
            ...
            <tbody id="table-body">
                ...
                <tr class="odd:bg-white even:bg-gray-50 border-b">
                    ...
                    <td class="px-6 py-4 font-extrabold">
                        {{ $s->nilaiPengetahuan->first()->nilai_pengetahuan ?? '-' }}
                    </td>

                    <td class="px-6 py-4 font-extrabold">
                        {{ $s->nilaiPengetahuan->first()->predikat ?? '-' }}
                    </td>
                    <td class="px-6 py-4">
                        <!-- Modal toggle -->
                        <button id="modal-toggle-{{ $s->id }}" data-modal-target="modal-{{ $s->id }}" data-modal-toggle="modal-{{ $s->id }}" @if($s->nilaiPengetahuan->first() && $s->nilaiPengetahuan->first()->nilai_pengetahuan)
                            disabled
                            @endif
                            class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 @if($s->nilaiPengetahuan->first() && $s->nilaiPengetahuan->first()->nilai_pengetahuan)
                            cursor-not-allowed
                                ...
                                <div class="relative bg-white rounded-lg shadow">
                                    ...
                                    <div class="p-4 md:p-5">
                                        <form class="space-y-4" action="{{ route('guru.input-nilai-pengetahuan') }}" method="POST">
                                            ...
                                            <div>
                                                <label for="nilai_pengetahuan" class="block mb-2 text-sm font-medium text-gray-900">Silahkan masukkan nilai pengetahuan</label>
                                                <input type="number" name="nilai_pengetahuan" id="nilai_pengetahuan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5" required/>
                                            </div>
