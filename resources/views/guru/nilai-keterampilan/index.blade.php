    ...
    <div class="relative overflow-x-auto shadow-md sm:rounded-lg p-6">
        <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
            <div>
                ...
                <form method="GET" action="{{ route('guru.filter-nilai-keterampilan') }}" class="mx-auto flex gap-4">
                    @csrf
            ...
            <div class="relative">
                <form action="{{ route('guru.cari-nilai-keterampilan') }}" method="GET">
                    @csrf
            ...
            <tbody id="table-body">
                ...
                <tr class="odd:bg-white even:bg-gray-50 border-b dark:bg-gray-800 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-600">
                    ...
                    <td class="px-6 py-4 font-extrabold">
                        {{ $s->nilaiketerampilan->nilai_keterampilan ?? '-' }}
                    </td>
                    <td class="px-6 py-4 font-extrabold">
                        {{ $s->nilaiketerampilan->predikat ?? '-' }}
                    </td>
                    <td class="px-6 py-4">
                        <!-- Modal toggle -->
                        <button id="modal-toggle-{{ $s->id }}" data-modal-target="modal-{{ $s->id }}" data-modal-toggle="modal-{{ $s->id }}" @if($s->nilaiKeterampilan && $s->nilaiKeterampilan->nilai_keterampilan)
                            disabled
                            @endif
                            class="focus:outline-none text-white bg-yellow-400 hover:bg-yellow-500 @if($s->nilaiKeterampilan && $s->nilaiKeterampilan->nilai_keterampilan)
                            cursor-not-allowed
                                ...
                                <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                    ...
                                    <div class="p-4 md:p-5">
                                        <form class="space-y-4" action="{{ route('guru.input-nilai-keterampilan') }}" method="POST">
                                            ...
                                            <div>
                                                <label for="nilai_keterampilan" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Silahkan masukkan nilai keterampilan</label>
                                                <input type="number" name="nilai_keterampilan" id="nilai_keterampilan" class="bg-gray-50 border border-gray-300 text-gray-900 text-sm rounded-lg focus:ring-blue-500 focus:border-blue-500 block w-full p-2.5 dark:bg-gray-600 dark:border-gray-500 dark:placeholder-gray-400 dark:text-white" required />
                                            </div>
