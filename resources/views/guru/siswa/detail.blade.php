    ...
    <div class="grid grid-cols-1 gap-4 mb-4">
        <div class="w-full flex p-4 gap-4 bg-white border border-gray-200 rounded-lg shadow sm:p-8 dark:bg-gray-800 dark:border-gray-700">
            ...
            <div class="flex flex-col gap-4">
                <h1 class="text-2xl font-bold">{{ $nilaiSiswa->siswa->nama_siswa }}</h1>
                <h2 class="text-xl text-gray-500">{{ $nilaiSiswa->siswa->nisn }}</h2>
                <h2 class="mb-2 text-lg text-gray-900">Hasil nilai <b>{{ $nilaiSiswa->mapel->nama_mapel_lengkap }}</b> selama satu semester di kelas <b>{{ $nilaiSiswa->kelas->nama_kelas }}</b> pada <b>Semester {{ $nilaiSiswa->semester_id }} tahun ajaran {{ $nilaiSiswa->tahun_ajaran }}</b>:</h2>
                <ul class="max-w-md space-y-1 text-gray-500 list-inside dark:text-gray-400">
                    <li class="flex items-center">
                        <svg class="w-3.5 h-3.5 me-2 {!! $nilaiSiswa ? 'text-green-500' : 'text-gray-500' !!} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                            <path d="M10 .5a9.5 9.5 0 1 0 9.5 9.5A9.51 9.51 0 0 0 10 .5Zm3.707 8.207-4 4a1 1 0 0 1-1.414 0l-2-2a1 1 0 0 1 1.414-1.414L9 10.586l3.293-3.293a1 1 0 0 1 1.414 1.414Z" />
                        </svg>
                        Nilai Pengetahuan: {!! $nilaiSiswa ? '<b>'.$nilaiSiswa->nilai_pengetahuan.'</b>' : '<a href="/guru/nilai-pengetahuan?cari='.$nilaiSiswa->id_siswa.'"><span class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded border border-red-400">Belum diberi nilai</span></a>'
                        !!}
                    </li>
                    <li class="flex items-center">
                        <svg class="w-3.5 h-3.5 me-2 {!! $nilaiSiswa ? 'text-green-500' : 'text-gray-500' !!} flex-shrink-0" aria-hidden="true" xmlns="http://www.w3.org/2000/svg" fill="currentColor" viewBox="0 0 20 20">
                        ...
                        Nilai Keterampilan: {!! $nilaiSiswa ? '<b>'.$nilaiSiswa->nilai_keterampilan.'</b>' : '<a href="/guru/nilai-keterampilan?cari='.$siswa->id.'"><span class="bg-red-100 text-red-800 text-xs font-medium me-2 px-2.5 py-0.5 rounded border border-red-400">Belum diberi nilai</span></a>' !!}
                    </li>
                </ul>
                @if($nilaiSiswa->status_lulus_mapel == 1)
                <div>
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow dark:bg-gray-700">
                                ...
                                <div class="p-4 md:p-5 text-center">
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah anda yakin akan memberikan kelulusan kepada siswa <b>{{ $nilaiSiswa->siswa->nama_siswa }}</b> dengan nilai pengetahuan <b>{!! $nilaiPengetahuan->nilai_pengetahuan ?? '-' !!}</b> dan nilai keterampilan <b>{!! $nilaiKeterampilan->nilai_keterampilan ?? '-' !!}</b> di kelas <b>{{ $nilaiSiswa->kelas->nama_kelas }}</b> pada <b>semester {{ $nilaiSiswa->semester_id }}</b> ?</h3>
                                    <form action="{{ route('guru.siswa.lulus') }}" method="POST" class="flex mx-auto justify-center gap-2">
                                        @csrf
                                        <input value="2" type="number" name="status_lulus_mapel" hidden>
                                        <input value="{{ $nilaiSiswa->id }}" type="number" name="id" hidden>
                        ...
                        <div class="relative p-4 w-full max-w-md max-h-full">
                            <div class="relative bg-white rounded-lg shadow">
                                ...
                                <div class="p-4 md:p-5 text-center">
                                    ...
                                    <h3 class="mb-5 text-lg font-normal text-gray-500 dark:text-gray-400">Apakah anda yakin akan tidak meluluskan siswa {{ $nilaiSiswa->siswa->nama_siswa }} ?</h3>
                                            ...
                                            <div class="relative bg-white rounded-lg shadow">
                                                ...
                                                <div class="p-4 md:p-5 text-center">
                                                    <form action="{{ route('guru.siswa.lulus') }}" method="post">
                                                        @csrf
                                                        <input value="3" type="number" name="status_lulus_mapel" hidden>
                                                        <input value="{{ $nilaiSiswa->id }}" type="number" name="id" hidden>
                                                        <label for="message" class="block mb-2 text-sm font-medium text-gray-900 dark:text-white">Alasan:</label>
                                                        <textarea id="message" name="alasan_tidak_lulus" rows="4" class="block mb-4 p-2.5 w-full text-sm text-gray-900 bg-gray-50 rounded-lg border border-gray-300 focus:ring-blue-500 focus:border-blue-500"></textarea>
                ...
                @else
                <div class="flex items-center p-4 mb-4 text-sm {!! $nilaiSiswa->status_lulus_mapel == 2 ? 'text-green-800 bg-green-50 border-green-300' : 'text-red-800 bg-red-50 border-red-300' !!} border rounded-lg" role="alert">
                    ...
                    <div>
                        ...
                        {!! $nilaiSiswa->status_lulus_mapel == 2 ? '<b>lulus</b>' : '<b>tidak lulus</b>' !!}
                        oleh <b>{{ $nilaiSiswa->guru->nama_guru }}</b>
                        {!! $nilaiSiswa->status_lulus_mapel == 3 ? ' dengan alasan ' . $nilaiSiswa->alasan_tidak_lulus : '' !!}
                    </div>
                </div>
                @endif
            </div>
