...
<div class="p-6 mt-14 space-y-4">
    <h1 class="text-2xl font-semibold">Nilai Keterampilan</h1>
    <div class="flex flex-column sm:flex-row flex-wrap space-y-4 sm:space-y-0 items-center justify-between pb-4">
        <div class="flex gap-4">
            <form method="GET" action="{{ route('kepsek.nilai-keterampilan') }}" class="mx-auto flex gap-4">
            ...
            <a href="{{ route('kepsek.nilai-keterampilan') }}" class="flex">
        ...
        <div class="relative">
            <form action="{{ route('kepsek.nilai-keterampilan') }}" method="GET">
    ...
    <div class="relative border border-gray-400 overflow-x-auto shadow-md sm:rounded-lg">
        <table class="w-full text-sm text-left rtl:text-right text-gray-500">
            ...
            <tbody id="table-body">
                @foreach($nilaiKeterampilan as $key => $n)
                <tr class="odd:bg-white even:bg-gray-50 border-b">
                    <th scope="row" class="px-6 py-4 font-medium text-gray-900 whitespace-nowrap">
                        {{ $nilaiKeterampilan->firstItem() + $key }}
                    </th>
                    <td class="px-6 py-4">
                        {{ $n->siswa->nama_siswa ?? '-' }}
                    </td>
